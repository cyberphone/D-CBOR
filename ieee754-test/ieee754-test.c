...
void oneTurn(char* valueText, char* cborHex) {
    ...
    double value = 0;
    if (!strcmp(valueText, "NaN")) {
        value = NAN;
    } else if (!strcmp(valueText, "Infinity")) {
        value = INFINITY;
    } else if (!strcmp(valueText, "-Infinity")) {
        value = -INFINITY;
    } else {
...
int main(int argc, const char* argv[]) {
    ...
    // The double values are given in "Diagnostic Notation" a la https://cbor.me
